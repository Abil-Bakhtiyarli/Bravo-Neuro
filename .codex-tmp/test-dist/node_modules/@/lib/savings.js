...
exports.calculateRecommendationSavings = calculateRecommendationSavings;
exports.attachSavingsToRecommendation = attachSavingsToRecommendation;
exports.summarizeBranchSavings = summarizeBranchSavings;
...
function getLookupKey(branchId, productId) {
    return `${branchId}:${productId}`;
}
function getClearanceWindowDays(record) {
    return Math.max(record.daysUntilEarliestExpiry, 1);
}
function getBaselineUnitsWithoutAction(record) {
    return roundUnits(Math.min(record.totalStock, record.salesHistory.avgDailySales * getClearanceWindowDays(record)));
}
function getUnitsAtRisk(record, baselineUnitsWithoutAction) {
    return roundUnits(Math.max(record.totalStock - baselineUnitsWithoutAction, 0));
}
function buildSavings(possibleLossAzN, recoveredValueAzN, netSavedValueAzN, estimatedRecoveredUnits, estimatedWasteUnitsAvoided, costBreakdown, assumptions) {
    return {
...
function calculateDiscountSavings(record, recommendation) {
    ...
    const baselineExpectedUnits = Math.min(record.totalStock, record.salesHistory.avgDailySales * (1 + DEFAULT_DISCOUNT_BASELINE_RESPONSE_RATE));
    ...
    return buildSavings(possibleLossUnits * record.product.unitPrice, recoveredValueAzN, recoveredValueAzN - discountCostAzN, recoveredUnits, recoveredUnits, {
        discountCostAzN,
        transferCostAzN: 0,
        handlingCostAzN: 0,
    }, {
        methodology: "Discount possible loss follows the requested Part 5 formula and uses the recommendation sell-through forecast as the at-risk baseline.",
        baselineUnitsWithoutAction: baselineExpectedUnits,
        unitsAtRisk: possibleLossUnits,
        discountedUnitPriceAzN,
    });
}
function calculateTransferSavings(record, recommendation) {
...
function calculateReorderSavings(record, recommendation) {
    ...
    return buildSavings(preventedUnits * record.product.unitPrice, recoveredValueAzN, recoveredValueAzN, preventedUnits, preventedUnits, {
        discountCostAzN: 0,
        transferCostAzN: 0,
        handlingCostAzN: 0,
    }, {
        methodology: "Reorder value estimates future waste prevention from excess coverage above the seven-day target.",
...
function calculateShelfActionSavings(record, recommendation) {
    ...
    const recoveredUnits = Math.min(unitsAtRisk, baselineUnitsWithoutAction * DEFAULT_SHELF_ACTION_UPLIFT_RATE);
    const recoveredValueAzN = recoveredUnits * record.product.unitPrice;
    return buildSavings(unitsAtRisk * record.product.unitPrice, recoveredValueAzN, recoveredValueAzN, recoveredUnits, recoveredUnits, {
        discountCostAzN: 0,
        transferCostAzN: 0,
        handlingCostAzN: 0,
    }, {
...
function calculateInvestigationSavings(record) {
    const baselineUnitsWithoutAction = getBaselineUnitsWithoutAction(record);
    const unitsAtRisk = getUnitsAtRisk(record, baselineUnitsWithoutAction);
    return buildSavings(unitsAtRisk * record.product.unitPrice, 0, 0, 0, 0, {
        discountCostAzN: 0,
        transferCostAzN: 0,
        handlingCostAzN: 0,
    }, {
        methodology: "Investigation is diagnostic only, so the record keeps its loss exposure but does not claim monetary recovery.",
        baselineUnitsWithoutAction,
        unitsAtRisk,
    });
}
function calculateRecommendationSavings(record, recommendation) {
...
function summarizeBranchSavings(records, recommendations) {
    const recordsByLookup = new Map(records.map((record) => [getLookupKey(record.branch.branchId, record.product.productId), record]));
    const summary = {
        totalPossibleLossAzN: 0,
        totalRecoveredValueAzN: 0,
        totalNetSavedValueAzN: 0,
    ...
    for (const actionType of Object.keys(summary.breakdownByActionType)) {
