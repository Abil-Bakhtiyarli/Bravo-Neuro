...
exports.generateRecommendationExplanation = generateRecommendationExplanation;
exports.attachExplanationToRecommendation = attachExplanationToRecommendation;
exports.attachExplanationsToRecommendations = attachExplanationsToRecommendations;
...
function getSelectedDrivers(record) {
    const nonZeroDrivers = record.risk.mainDrivers.filter((driver) => driver.weightedContribution > 0);
    return nonZeroDrivers.slice(0, Math.min(nonZeroDrivers.length, 3));
}
function getRequiredDailySales(record) {
...
function formatStockPressure(record) {
...
function formatSalesWeakness(record) {
...
function formatHistoricalWaste(record) {
...
function formatBranchDemandMismatch(record) {
    ...
    const speedMultiplier = roundToOne(bestAlternateBranch.avgDailySales / record.salesHistory.avgDailySales);
    return `${bestAlternateBranch.branchName} sells this product ${speedMultiplier}x faster than ${record.branch.branchName}`;
}
function formatDriverHighlight(driver, record) {
    switch (driver.key) {
        case "expiry-urgency":
        ...
        default: {
            const exhaustiveCheck = driver.key;
            throw new Error(`Unsupported risk driver: ${String(exhaustiveCheck)}`);
        }
    }
}
function buildSummary(record, driverHighlights) {
    const clauses = driverHighlights.map((highlight) => highlight.replace(`${record.product.name} `, "").replace(/\.$/, ""));
...
function buildRecommendationRationale(record, recommendation) {
    switch (recommendation.actionType) {
        case "discount": {
            const fallbackText = recommendation.fallbackDiscountPercent > recommendation.discountPercent
...
function generateRecommendationExplanation(record, recommendation) {
    const selectedDrivers = getSelectedDrivers(record);
    const driverHighlights = selectedDrivers.map((driver) => formatDriverHighlight(driver, record));
    return {
...
function attachExplanationToRecommendation(record, recommendation) {
    return {
        recommendation,
        explanation: generateRecommendationExplanation(record, recommendation),
    };
}
function attachExplanationsToRecommendations(records, recommendations) {
    const recordsByLookup = new Map(records.map((record) => [createLookupKey(record.branch.branchId, record.product.productId), record]));
