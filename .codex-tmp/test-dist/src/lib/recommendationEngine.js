...
exports.generateRecommendationsForBranch = generateRecommendationsForBranch;
const ELIGIBLE_RISK_LEVELS = ["medium", "high", "critical"];
...
function getExcessStock(record) {
...
function getDestinationExpectedDemandUntilExpiry(destination, record) {
    const expiryWindowDays = Math.max(record.daysUntilEarliestExpiry, 1);
    return Math.max(destination.avgDailySales * expiryWindowDays - destination.totalStock, 0);
}
function getDiscountPercent(record) {
...
function needsInvestigation(record) {
    return (record.salesHistory.avgDailySales <= 0 ||
        record.daysOfStockRemaining === null ||
        record.daysUntilEarliestExpiry < 0);
}
function buildInvestigationRecommendation(record, reasonCodes, summaryReasons) {
    return {
...
function getTransferRecommendation(record) {
    ...
    const destinationExpectedDemandUntilExpiry = getDestinationExpectedDemandUntilExpiry(bestAlternateBranch, record);
    const transferQuantity = Math.min(excessStock, destinationExpectedDemandUntilExpiry);
    if (transferQuantity < 1) {
        return undefined;
    }
    const speedMultiplier = currentSales > 0 ? Number((bestAlternateBranch.avgDailySales / currentSales).toFixed(2)) : 0;
    return {
        branchId: record.branch.branchId,
        productId: record.product.productId,
        branchName: record.branch.branchName,
...
function getDiscountRecommendation(record) {
    ...
    const fallbackDiscountPercent = expectedUnitsByTomorrow < targetUnitsByTomorrow
...
function getReorderRecommendation(record) {
    if (record.daysUntilEarliestExpiry <= 4) {
        return undefined;
    }
    const hasOversupply = (record.daysOfStockRemaining ?? 0) >= 10;
    const hasWasteTrend = record.wasteHistory.previousWasteUnits >= 5 || record.wasteHistory.categoryWasteRate >= 0.12;
...
function getShelfActionRecommendation(record) {
    if (record.daysUntilEarliestExpiry > 4) {
        return undefined;
    }
    const targetPlacement = record.inventoryLots.some((lot) => lot.shelfLocation !== "promo endcap")
        ? "promo endcap"
        : "front shelf";
    return {
        branchId: record.branch.branchId,
...
function generateRecommendation(record) {
    if (!hasEligibleRiskLevel(record.risk.riskLevel)) {
        return undefined;
    }
    if (needsInvestigation(record)) {
        return buildInvestigationRecommendation(record, ["data-conflict"], ["sales or stock coverage data is unusable"]);
    }
    ...
    return buildInvestigationRecommendation(record, ["unclear-primary-action"], ["the risk is elevated but the best next step is not conclusive"]);
}
function generateRecommendationsForBranch(records) {
    return records
        .map((record) => ({
        record,
        recommendation: generateRecommendation(record),
    }))
        .filter((entry) => entry.recommendation !== undefined)
        .sort((left, right) => {
        const riskPriorityDelta = RISK_LEVEL_PRIORITY[right.record.risk.riskLevel] -
