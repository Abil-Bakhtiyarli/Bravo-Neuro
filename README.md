...
Part 19 is complete: `data/monthly_savings_history.json`, `src/lib/seedData.ts`, `src/lib/dashboardData.ts`, `src/lib/types.ts`, `src/components/MonthlySavingsChart.tsx`, `src/app/page.tsx`, and the active presentation surfaces now add a real branch-scoped monthly net-saved chart above the KPI rail, validate seeded monthly history, and polish stale copy, badges, spacing, and empty states so the demo feels branch-aware instead of staged. The current seeded history window spans 12 months and varies month to month instead of reading like a flat trend line.
...
Part 002 is complete as the dashboard overview homepage pivot: `src/app/page.tsx` now keeps server-side branch resolution and snapshot loading but replaces the old `DashboardHeader`, `DashboardLayout`, `KpiCards`, and `DashboardDemoExperience` homepage composition with a new overview surface rendered inside `src/components/DashboardOverview.tsx`. The homepage now uses `src/components/CompactDashboardHeader.tsx`, `src/components/SummaryKpiGrid.tsx`, `src/components/TodayDecisionCard.tsx`, `src/components/TopRiskProductsCard.tsx`, and `src/components/BranchComparisonCard.tsx` to present a compact executive summary with four branch-aware KPIs, the main savings chart, one primary AI decision, a top-five risk preview, and a read-only branch comparison card, while preserving the existing URL-backed `product` selection and shared `src/components/ProductRiskDrawer.tsx` proof modal. This pass intentionally removes the full `DailyActionPlan` and full `RiskTableExperience` from `/` without changing `src/lib` business logic, route structure, or branch query behavior, and the homepage-focused component coverage now shifts to the new overview surfaces instead of the older operations layout.

Part 003 is complete as the sidebar detail-pages pass: `src/lib/branchSelection.ts` now centralizes route-safe branch resolution, `src/lib/branchComparison.ts` now centralizes the shared branch-comparison summary derivation, and `src/components/Sidebar.tsx` now enables the real `/risk`, `/actions`, and `/branches` navigation items while intentionally keeping later sections disabled. `src/app/risk/page.tsx` now restores the full risk-analysis surface with the existing `RiskTableExperience` and shared product proof modal, `src/app/actions/page.tsx` now restores the full manager queue through a dedicated `ActionPlanExperience` wrapper plus the same modal detail flow, and `src/app/branches/page.tsx` now gives the comparison view its own route while preserving the clean Part 002 homepage on `/`. The shared product proof modal now uses App Router search-param navigation for open/close state and supports explicit close plus empty-space dismiss instead of relying on direct `window.history` mutations. This pass keeps `branch` as the canonical cross-page context, preserves URL-backed `product` detail behavior where relevant, adds route-level and sidebar/header coverage for the new pages, and `corepack.cmd pnpm lint`, `corepack.cmd pnpm test`, and `corepack.cmd pnpm build` are the required verification commands for the completed route split.

Part 004 is complete as the polish and copy-tightening pass: `src/components/Sidebar.tsx`, `src/components/CompactDashboardHeader.tsx`, and `src/components/DetailPageHeader.tsx` now remove the stale `Demo Mode` wording in favor of smaller live-data status treatments, while keeping branch/date context and URL-backed branch switching unchanged. `src/components/SummaryKpiGrid.tsx`, `src/components/MonthlySavingsChart.tsx`, `src/components/TodayDecisionCard.tsx`, `src/components/TopRiskProductsCard.tsx`, and `src/components/BranchComparisonCard.tsx` now reuse the shared animation helpers from `src/app/globals.css`, tighten helper copy, and align cards around one consistent radius, border, and shadow system without changing any `src/lib` dashboard contracts or the shared `product` modal flow. The route headers on `/actions` and `/branches` now use shorter English-only subtitles, the homepage keeps the same overview component set while reading faster in the first screen, the affected component tests now assert the new live-data copy instead of the removed demo labels, and `corepack.cmd pnpm lint`, `corepack.cmd pnpm test`, and `corepack.cmd pnpm build` pass after the presentation-only refinement.
...
- 158 products across dairy, bakery, fruits/vegetables, and drinks
...
`data/monthly_savings_history.json`, `src/lib/seedData.ts`, `src/lib/dashboardData.ts`, and `src/components/MonthlySavingsChart.tsx` are the canonical Part 19 branch-history surface. They add one validated shared monthly net-saved series per branch, preload that series into the server snapshot, and render it on the dashboard so branch switching updates the full story together. The current seed window is 12 months long and intentionally non-linear.
