...
Part 12 is complete: `src/components/RiskTable.tsx` now provides a polished presentation-only risk table with static filter shells, urgency styling, risk badges, and action labels while intentionally keeping the rows mocked until Part 13 connects the canonical `riskTable` payload.
Part 13 and later will activate live risk table data, drawer detail, task interaction, and only add route handlers if a public HTTP surface is still needed.
...
`src/components/RiskTable.tsx` is the canonical Part 12 risk-table presentation surface. It remains presentation-only: Part 12 establishes polished rows, risk/action visual mapping, and static control shells while Part 13 is responsible for replacing mock rows with the live branch `riskTable` contract from `src/lib/dashboardData.ts`.
