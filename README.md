...
Part 003 is complete as the sidebar detail-pages pass: `src/lib/branchSelection.ts` now centralizes route-safe branch resolution, `src/lib/branchComparison.ts` now centralizes the shared branch-comparison summary derivation, and `src/components/Sidebar.tsx` now enables the real `/risk`, `/actions`, and `/branches` navigation items while intentionally keeping later sections disabled. `src/app/risk/page.tsx` now restores the full risk-analysis surface with the existing `RiskTableExperience` and shared product proof modal, `src/app/actions/page.tsx` now restores the full manager queue through a dedicated `ActionPlanExperience` wrapper plus the same modal detail flow, and `src/app/branches/page.tsx` now gives the comparison view its own route while preserving the clean Part 002 homepage on `/`. This pass keeps `branch` as the canonical cross-page context, preserves URL-backed `product` detail behavior where relevant, adds route-level and sidebar/header coverage for the new pages, and `corepack.cmd pnpm lint`, `corepack.cmd pnpm test`, and `corepack.cmd pnpm build` are the required verification commands for the completed route split.
