...
export function ProductRiskDrawerContent({
  detail,
  withinDialog = true,
}: {
  ...
  return (
    <div className="flex h-[min(100%,56rem)] w-full max-w-6xl flex-col overflow-hidden rounded-[2rem] border border-border/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.985),rgba(248,250,252,0.98))] shadow-[0_38px_110px_-42px_rgba(15,23,42,0.85)]">
      <div
        className={cn(
          "border-b border-border/80 bg-gradient-to-br px-5 py-5 sm:px-6 lg:px-7",
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            {withinDialog ? (
              <Dialog.Description className="mt-3 max-w-3xl text-sm leading-6 text-foreground/78">
                Review why this item is exposed, which action the branch should take first, and how
                much value is still recoverable from the current situation.
              </Dialog.Description>
            ) : (
              <p className="mt-3 max-w-3xl text-sm leading-6 text-foreground/78">
                Review why this item is exposed, which action the branch should take first, and how
                much value is still recoverable from the current situation.
              </p>
            )}
          ...
          <div className="flex items-start gap-2">
            ...
            {withinDialog ? (
              ...
              <Button aria-label="Close product detail modal" size="icon-sm" variant="outline">
                <X className="size-4" />
              </Button>
            )}
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5 sm:px-6 lg:px-7 lg:py-6">
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(21rem,0.85fr)] xl:items-start">
