import type { ActionPlanItem, ProductRecommendation, RiskLevel } from "./types";

type ActionPlanSortShape = Pick<
  ActionPlanItem,
  "riskLevel" | "expectedNetSavedValueAzN" | "daysUntilExpiry" | "productName" | "taskId"
>;

const ACTION_PLAN_RISK_PRIORITY: Record<RiskLevel, number> = {
  critical: 3,
  high: 2,
  medium: 1,
  low: 0,
...
function formatExpiryWindow(daysUntilExpiry: number) {
...
export function buildActionPlanChecklistSteps(
...
export function compareActionPlanItems(left: ActionPlanSortShape, right: ActionPlanSortShape) {
  const riskDelta =
    ACTION_PLAN_RISK_PRIORITY[right.riskLevel] - ACTION_PLAN_RISK_PRIORITY[left.riskLevel];
...
export function rankActionPlanItems<T extends Omit<ActionPlanItem, "priorityRank">>(
  items: readonly T[],
): ActionPlanItem[] {
  return [...items]
