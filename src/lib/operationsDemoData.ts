import { getAvailableBranches, getBranchProductRecords } from "./dataLoader";
...
export type TransferLaneUrgency = "critical" | "watch" | "planned";
...
export type TransferLane = {
  ...
  expectedValueProtectedAzN: number;
  transferWindowLabel: string;
  statusLabel: string;
  urgency: TransferLaneUrgency;
  summary: string;
};
...
export type TransfersPageData = {
  ...
  valueProtectedAzN: number;
  unitsToMove: number;
  urgentTransferCount: number;
  lanes: TransferLane[];
};

export type ForecastDriverTone = "upside" | "risk" | "watch";

export type ForecastSummaryMetric = {
  label: string;
  value: string;
  helperText: string;
  tone: ForecastDriverTone;
};

export type ForecastPoint = {
  weekKey: string;
  weekLabel: string;
  projectedRevenueAzN: number;
  baselineRevenueAzN: number;
};
...
export type RevenueForecastPageData = {
  generatedAt: string;
  branchName: string;
...
type BranchForecastProfile = {
  weeklyMultipliers: [number, number, number, number];
  confidencePercent: number;
  topOpportunity: string;
  topRisk: string;
  driverTemplates: ForecastDriver[];
  categoryAdjustments: Partial<Record<ProductCategory, number>>;
};
...
const forecastProfilesByBranch: Record<BranchId, BranchForecastProfile> = {
  ganjlik: {
...
const fallbackTransferLanesByBranch: Record<BranchId, TransferLane[]> = {
  ganjlik: [
    {
...
function buildTransferWindowLabel(daysUntilExpiry: number, urgency: TransferLaneUrgency) {
  if (urgency === "critical") {
...
function buildTransferStatusLabel(direction: TransferDirection, urgency: TransferLaneUrgency) {
  if (urgency === "critical") {
...
function buildTransferUrgency(daysUntilExpiry: number, speedMultiplier: number): TransferLaneUrgency {
...
function getQuantityToTransfer(
...
function buildTransferCandidates(): TransferCandidate[] {
  const branches = getAvailableBranches();
  const allRecords = branches.flatMap((branch) => getBranchProductRecords(branch.branchId));
...
function toTransferLane(
  candidate: TransferCandidate,
  branchId: BranchId,
): TransferLane {
  const direction: TransferDirection =
    candidate.sourceBranchId === branchId ? "outbound" : "inbound";
  const urgency = buildTransferUrgency(
    candidate.daysUntilExpiry,
    candidate.speedMultiplier,
  );
  const counterpartBranchName =
    direction === "outbound"
...
function compareTransferLanes(left: TransferLane, right: TransferLane) {
...
export function getTransfersPageData(branchId: BranchId): TransfersPageData {
  const dashboardData = getDashboardData(branchId);
  const computedLanes = buildTransferCandidates()
    .filter(
      (candidate) =>
        candidate.sourceBranchId === branchId || candidate.destinationBranchId === branchId,
    )
  ...
  return {
    ...
    valueProtectedAzN: roundToTwo(
      lanes.reduce((sum, lane) => sum + lane.expectedValueProtectedAzN, 0),
    ),
...
function getBranchBaseWeeklyRevenue(branchId: BranchId) {
  return getBranchProductRecords(branchId).reduce(
    (total, record) => total + record.salesHistory.avgDailySales * 7 * record.product.unitPrice,
    0,
  );
}

function getCategoryWeeklyRevenue(branchId: BranchId) {
  const totals = new Map<ProductCategory, number>();

  for (const record of getBranchProductRecords(branchId)) {
    const currentValue = totals.get(record.product.category) ?? 0;
    totals.set(
      record.product.category,
      currentValue + record.salesHistory.avgDailySales * 7 * record.product.unitPrice,
    );
  }
...
export function getRevenueForecastPageData(branchId: BranchId): RevenueForecastPageData {
  const dashboardData = getDashboardData(branchId);
  ...
  const projectedFourWeekRevenue = chartPoints.reduce(
    (total, point) => total + point.projectedRevenueAzN,
    0,
  );
  const averageProjectedWeek =
    projectedFourWeekRevenue / Math.max(chartPoints.length, 1);
  const weekFourChangePercent =
    ((chartPoints[3].projectedRevenueAzN - chartPoints[0].baselineRevenueAzN) /
      chartPoints[0].baselineRevenueAzN) *
    100;
  const opportunityTone: ForecastDriverTone =
    weekFourChangePercent >= 0 ? "upside" : "watch";
  const categoryBreakdown = [...getCategoryWeeklyRevenue(branchId).entries()]
    .map(([category, weeklyRevenue]) => {
      const categoryMultiplier =
        profile.categoryAdjustments[category] ?? profile.weeklyMultipliers[3];
      const projectedRevenueAzN = roundToOne(weeklyRevenue * categoryMultiplier);
      const changePercent =
        ((projectedRevenueAzN - weeklyRevenue) / Math.max(weeklyRevenue, 1)) * 100;
...
export const OPERATIONS_DEMO_REFERENCE_DATE = toGeneratedAt(SEED_REFERENCE_DATE);
